...
export const issuesApi = {
  ...
  create: (projectId: string, data: { title: string; description?: string; riskLevel?: IssueRisk; status?: IssueStatus; assigneeId?: string; taskId?: string }) =>
    api.post<Issue>(`/projects/${projectId}/issues`, data).then((r) => r.data),
  update: (projectId: string, issueId: string, data: Partial<{ title: string; description: string; riskLevel: IssueRisk; status: IssueStatus; assigneeId: string | null; taskId: string | null }>) =>
